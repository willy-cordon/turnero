...
@section('styles')
    <title>{{ config('app.name') }} | {{ trans('cruds.role.title_singular') }}</title>
@endsection
@section('content')
    ...
    <div class="card-body">
        <form action="{{ route("admin.roles.update", [$role->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('permission') ? 'has-error' : '' }}">
                ...
                <select name="permission[]" id="permission" class="form-control to-select2" multiple="multiple" required>
